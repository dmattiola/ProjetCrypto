...
public class Pallier extends CryptoSysteme {
    ...
    public void generateKeys(){
        ...
        super.setSk((p.subtract(new BigInteger("1"))).multiply((q.subtract(new BigInteger("1")))));
    }

    @Override
    public BigInteger encrypter(BigInteger m) {
        if (m.compareTo(BigInteger.ZERO) < 0 || m.compareTo(super.getPk()) >= 0){
            return null;
        }
        BigInteger r;
        do {
        ...
        BigInteger debut = super.getPk().add(new BigInteger("1")).modPow(m, super.getPk().multiply(super.getPk()));
